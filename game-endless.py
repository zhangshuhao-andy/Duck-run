...
GAME_DATA = {"coins": 5000, "current_color": C_GOLD, "owned_skins": ["Classic Gold"]}
SHOP_ITEMS = [
...
def run_endless():
    ...
    # ⭐ 更简单：减少危险物出现率
         if choice < 0.4:   # 原来是0.7 → 降低
            hazards.append({'rect': pygame.Rect(curr_x, GROUND_Y-45, 42, 45), 'type': 'mine'})
            curr_x += random.randint(320, 480)  # ⭐ 拉大间距
         elif choice < 0.7:
            hazards.append({'rect': pygame.Rect(curr_x, GROUND_Y-120, 180, 35), 'type': 'laser'})  # ⭐ 缩短激光
            curr_x += 500
         else:
            # ⭐ 留空区域（让玩家喘气）
            curr_x += random.randint(300, 450)

    # ⭐ 平台更友好（更容易踩）
         if random.random() > 0.3:  # 原来0.5 → 更容易生成
            platforms.append(pygame.Rect(curr_x - 120, GROUND_Y-160, 220, 20))
